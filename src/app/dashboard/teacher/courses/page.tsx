...
import { PlusCircle, Edit, Trash2, RefreshCcw, ArrowLeft, Search, Filter, Calendar, Clock, Book, Loader2, X, ChevronDown, ChevronUp, Save } from 'lucide-react'
...
export default function CoursesPage() {
  ...
  const fetchCourses = async () => {
    ...
    if (!teacherId) {
      setError(t('dashboard.errors.teacherInfoNotFound'))
      setLoading(false)
      return
    }
    ...
    try {
      ...
      setError(error.message || t('common.networkError', 'Network error. Please check your connection.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const deleteCourse = async (id: number) => {
    ...
    try {
      ...
      showToast(t('coursesPage.courseDeleted', 'Course successfully deleted.'), 'success')
    } catch (error) {
      console.error("Error deleting course:", error)
      showToast(t('coursesPage.deleteError', 'Error deleting the course.'), 'error')
    } finally {
      setIsDeleting(false)
      setDeleteId(null)
    }
  ...
  const updateCourse = async () => {
    ...
    try {
      ...
      showToast(t('coursesPage.courseUpdated', 'Course successfully updated.'), 'success')
    } catch (error) {
      console.error("Error updating course:", error)
      showToast(t('coursesPage.updateError', 'Error updating the course.'), 'error')
    } finally {
      setIsEditing(false)
    }
  }

  // Re-added showToast function
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-white dark:bg-gray-900 min-h-screen">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6 shadow-xl">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{t('coursesPage.confirmDelete', 'Are you sure you want to delete?')}</h3>
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {isDeleting ? t('common.loading', 'Loading...') : t('coursesPage.yesDelete', 'Yes, Delete')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-medium text-gray-900 dark:text-white">
                {t('coursesPage.editTitle', 'Edit')}: {editingCourse.name}
              </h3>
              <button 
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('coursesPage.courseCode', 'Course Code')}
                </label>
                <input 
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('coursesPage.courseName', 'Course Name')}
                </label>
                <input 
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('coursesPage.semester', 'Semester')}
                </label>
                <input 
                  type="text"
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {t('coursesPage.lessonTimes', 'Lesson Times')}
                  </label>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    {t('coursesPage.addTime', 'Add Time')}
                  </button>
                </div>
                  ...
                  <div key={index} className="p-3 mb-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-800/70">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        {t('coursesPage.lessonNum', 'Lesson #{{index}}', { index: index + 1 })}
                      </span>
                      <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                          {t('coursesPage.startTime', 'Start Time')}
                        </label>
                        <input 
                          type="time"
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                          {t('coursesPage.endTime', 'End Time')}
                        </label>
                        <input 
                          type="time"
                          value={time.end_time}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('coursesPage.title', 'Courses')}</h1>
          <p className="text-gray-600 dark:text-gray-400">{t('coursesPage.description', 'Manage all your courses...')}</p>
        </div>
        <div className="flex mt-4 md:mt-0 space-x-3">
          ...
          >
            ...
            {t('coursesPage.backToDashboard', 'Back to Dashboard')}
          </Link>
          <Link
          ...
          >
            ...
            {t('coursesPage.createNewCourse', 'Create New Course')}
          </Link>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            title={t('coursesPage.refreshCourses', 'Refresh Courses')}
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">{t('coursesPage.noCoursesFound', 'No courses found.')}</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6">{searchTerm ? t('coursesPage.noCoursesMatchSearch', 'No courses match your search criteria.') : ''}</p>
          {!searchTerm && (
            <Link
              href="/dashboard/teacher/courses/new"
            ...
            >
              <PlusCircle className="mr-2 h-4 w-4" />
              {t('coursesPage.createFirst', 'Create your first course!')}
            </Link>
          )}
        </div>
      ) : (
              ...
              <div className="p-5">
                  ...
                  {course.lesson_times && course.lesson_times.length > 0 ? (
                    course.lesson_times.map((time, idx) => (
                      <div key={idx} className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <span>{t(`days.${time.day}`, time.day)}</span>
                        <Clock className="h-4 w-4 mx-1 flex-shrink-0" />
                        <span>{time.start_time} - {time.end_time}</span>
                      </div>
                    ))
                  ) : (
                    <div className="text-sm text-gray-500 dark:text-gray-400 italic">{t('coursesPage.noLessonTimes', 'No lesson times defined')}</div>
                  )}
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                  ...
                  <div className="flex space-x-2">
                    <button onClick={() => openEditModal(course)} className="inline-flex items-center p-1.5 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600" title={t('common.edit', 'Edit')}>
                      <Edit className="h-4 w-4" />
                    </button>
                    <button onClick={() => setDeleteId(course.id)} className="inline-flex items-center p-1.5 text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 rounded-md hover:bg-red-200 dark:hover:bg-red-800/30" title={t('common.delete', 'Delete')}>
